...
static char nick[64], *server;
static int quit, winchg, rawlog, id;
...
dial(char *host, char *port)
{
	...
	if(getaddrinfo(host, port, &hints, &res) != 0)
		panic("error: cannot resolve hostname '%s':", host);
	...
	freeaddrinfo(res);
	if(!r)
		panic("error: cannot connect to host '%s'\n", host);
	return srv;
}
...
scmd(char *usr, char *cmd, char *par, char *data)
{
	...
	if (!strcmp(cmd, "PRIVMSG") || !strcmp(cmd, "NOTICE")) {
		if (!pm || !data)
			return;
		if(!strcmp(pm, nick)) {
			if(strcmp(usr, "NickServ") == 0 && id == 0)
			if(strstr(data, "You are now identified for"))
				id++;
			c = chfind(usr);
		} else
			c = chfind(pm);
...
uparse(char *m)
{
	...
	case 'n': /* Change nick. */
		p += 1 + (p[1] == ' ');
		if(strlen(p) >= sizeof(nick)) {
			pushf(ch, PFMT, nick, "Nick too long");
			return;
		}
		strcpy(nick, p);
		sndf("NICK %s", nick);
...
main(int argc, char *argv[])
{
	...
	server = SRV;
	while ((o = getopt(argc, argv, "hrn:u:s:p:l:P:")) >= 0)
		switch (o) {
		case 'h':
		case '?':
		usage:
	...
	sndf("MODE %s +i", nick);
	if(pass != NULL)
		sndf("PRIVMSG NickServ :identify %s", pass);
	while (!quit) {
		if(id == 1) {
			for(ch = chans; *ch != NULL; ch++) {
				chadd(*ch);
				sndf("JOIN %s", *ch);
			}
			id++;
